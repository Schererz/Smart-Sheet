...
def criar_aposta(aposta: schemas.BetCreate, db: Session = Depends(get_db)):
    return crud.criar_aposta(db, aposta)


@router.get("/", response_model=list[schemas.BetOut])
def listar_apostas(skip: int = 0, limit: int = 200, db: Session = Depends(get_db)):
    return crud.listar_apostas(db, skip, limit)
...
def evolucao_banca(db: Session = Depends(get_db)):
    return crud.obter_evolucao_banca(db)
...
def calcular_retorno_turbinado(dados: schemas.CalcularRetornoRequest):
    ...
    formulário enquanto o usuário digita."""
...
def obter_aposta(aposta_id: int, db: Session = Depends(get_db)):
    aposta = crud.obter_aposta(db, aposta_id)
    if not aposta:
...
def atualizar_aposta(aposta_id: int, aposta: schemas.BetUpdate, db: Session = Depends(get_db)):
    atualizada = crud.atualizar_aposta(db, aposta_id, aposta)
    if not atualizada:
        raise HTTPException(status_code=404, detail="Aposta não encontrada")
    return atualizada
...
def deletar_aposta(aposta_id: int, db: Session = Depends(get_db)):
    deletada = crud.deletar_aposta(db, aposta_id)
    if not deletada:
        raise HTTPException(status_code=404, detail="Aposta não encontrada")
    return {"ok": True}
